...
def _extract_roi(src_ori, i):
    ...
    most_left = int(white_xs[0]) + 15
    most_right = min(int(cx + (cx - most_left) * 1), int(white_xs[-1]))
    ...
    most_top = np.nonzero(overall_center_column)[0][0] - 15
    most_bottom = np.nonzero(overall_center_column)[0][-1] + 5
